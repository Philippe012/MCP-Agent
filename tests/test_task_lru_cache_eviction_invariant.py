...
def seeded_workspace(tmp_path):
    ws = make_episode_workspace(base_dir=tmp_path, episode_id="test-lru-seed", task_id=TASK_ID)
    yield ws
    cleanup(ws)


def test_seed_get_does_not_touch_recency(seeded_workspace):
    text = (seeded_workspace / "src" / "cache" / "lru.py").read_text(encoding="utf-8")
    get_body = text.split("def get(self, key):")[1].split("def put")[0]
    assert "_touch" not in get_body
...
def test_seed_workspace_contains_only_sandboxed_files(seeded_workspace):
    ...
    assert "src/cache/lru.py" in names
    assert "tasks/lru_cache_eviction_invariant/task.md" in names
    assert not any(n.startswith("tasks/") and n != "tasks/lru_cache_eviction_invariant/task.md" for n in names)
...
def test_verifier_scores_unfixed_seed_below_full_reward(seeded_workspace):
    ...
    assert report["behavior_passed"] is False  # get() doesn't protect recency


def test_verifier_scores_the_real_fix_plus_regression_test_at_full_reward(seeded_workspace):
    fixed = (REPO_ROOT / "src" / "cache" / "lru.py").read_text(encoding="utf-8")
    (seeded_workspace / "src" / "cache" / "lru.py").write_text(fixed, encoding="utf-8")

    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text(
        (REPO_ROOT / "tests" / "test_task_regression_lru.py").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_verifier_rejects_a_vacuous_regression_test(seeded_workspace):
    ...
    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text("def test_cache():\n    assert True\n", encoding="utf-8")
