...
def seeded_workspace(tmp_path):
    ws = make_episode_workspace(base_dir=tmp_path, episode_id="test-contact-seed", task_id=TASK_ID)
    yield ws
    cleanup(ws)
...
def test_seed_workspace_contains_only_sandboxed_files(seeded_workspace):
    ...
    assert "src/contact_index/directory.py" in names
    assert "src/contact_index/__init__.py" in names
...
def test_verifier_scores_the_real_fix_plus_regression_test_at_full_reward(seeded_workspace):
    fixed = (REPO_ROOT / "src" / "contact_index" / "directory.py").read_text(encoding="utf-8")
    (seeded_workspace / "src" / "contact_index" / "directory.py").write_text(fixed, encoding="utf-8")

    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text(
        (REPO_ROOT / "tests" / "test_task_regression_contact_index.py").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_verifier_rejects_a_vacuous_regression_test(seeded_workspace):
    ...
    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text("def test_contacts():\n    assert True\n", encoding="utf-8")
