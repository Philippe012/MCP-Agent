...
def seeded_workspace(tmp_path):
    ws = make_episode_workspace(base_dir=tmp_path, episode_id="test-calendar-seed", task_id=TASK_ID)
    yield ws
    cleanup(ws)


def test_seed_has_the_inclusive_boundary_bug(seeded_workspace):
    text = (seeded_workspace / "src" / "scheduler" / "calendar.py").read_text(encoding="utf-8")
    assert "start <= b.end and b.start <= end" in text
...
def test_seed_workspace_contains_only_sandboxed_files(seeded_workspace):
    ...
    assert "src/scheduler/calendar.py" in names
    assert "tasks/calendar_booking_overlap/task.md" in names
    assert not any(n.startswith("tasks/") and n != "tasks/calendar_booking_overlap/task.md" for n in names)
...
def test_verifier_scores_unfixed_seed_below_full_reward(seeded_workspace):
    ...
    assert report["behavior_passed"] is False  # touching bookings wrongly rejected


def test_verifier_scores_the_real_fix_plus_regression_test_at_full_reward(seeded_workspace):
    fixed = (REPO_ROOT / "src" / "scheduler" / "calendar.py").read_text(encoding="utf-8")
    (seeded_workspace / "src" / "scheduler" / "calendar.py").write_text(fixed, encoding="utf-8")

    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text(
        (REPO_ROOT / "tests" / "test_task_regression_calendar.py").read_text(encoding="utf-8"),
        encoding="utf-8",
    )
...
def test_verifier_rejects_a_vacuous_regression_test(seeded_workspace):
    ...
    regression = seeded_workspace / "tests" / "test_task_regression.py"
    regression.write_text("def test_booking():\n    assert True\n", encoding="utf-8")
